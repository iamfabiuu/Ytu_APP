...
const PLACE = {
  ...
  distance: "850 m",
  description:
    "Um espaço vivo para conhecer, sentir e celebrar o frevo. Patrimônio Cultural Imaterial da Humanidade.",
  hours: { value: "10h às 17h", note: "Aberto agora" },
  ticket: { value: "R$ 10", note: "Grátis às terças" },
  accessibility: {
    title: "Espaço acessível",
    features: ["Elevador", "Banheiro adaptado", "Libras"],
  },
  address: { line1: "Praça do Arsenal", line2: "Bairro do Recife, Recife/PE" },
};
      ...
      <div className="relative z-10 -mt-8 rounded-t-[32px] bg-white px-5 pt-6">
        ...
        <div className="mt-5 grid grid-cols-2 gap-3">
          <InfoCard
            ...
            label="HORÁRIO"
            value={PLACE.hours.value}
            note={PLACE.hours.note}
          />
          <InfoCard
            icon={<Wallet className="size-6" />}
            label="ENTRADA"
            value={PLACE.ticket.value}
            note={PLACE.ticket.note}
          />
        </div>
        ...
        <div className="mt-4 rounded-3xl bg-[#E8EEFF] p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-[#0B1B3F]">
                {PLACE.accessibility.title}
              </p>
            </div>
          </div>
          ...
          <ul className="mt-3 flex flex-wrap gap-2">
            {PLACE.accessibility.features.map((f) => (
              <li
                key={f}
                className="rounded-full bg-[#1D3FA8] px-4 py-2 text-xs font-bold text-white"
              >
                {f}
              </li>
            ))}
        ...
        {/* COMO CHEGAR */}
        <div className="mt-6">
          <p className="text-[11px] font-extrabold tracking-wide text-[#DC4A0C]">
            COMO CHEGAR
          </p>
          <h2 className="mt-1 text-2xl font-bold text-[#0B1B3F]">
            {PLACE.address.line1}
          </h2>
          <div className="mt-1 flex items-center justify-between gap-3">
            ...
            <button className="text-sm font-bold text-[#0B1B3F] underline underline-offset-2 active:scale-95">
              Ver mapa
            </button>
          </div>
        </div>
